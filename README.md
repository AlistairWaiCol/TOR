...
- **`seed:map` only works against a database on the same machine as the app serving its files —
  running it against Railway's Postgres from a local machine was tried once and produced a
  calibration row pointing at `.webp` files that only existed locally, 404ing for every player on the
  live site.** The map's derivatives are generated by `sharp` writing to local disk (`uploads/`),
  entirely separate from the database row that records their filenames — seeding the DB remotely
  doesn't move the files. Caught immediately (the bad row was deleted before anyone but the GM saw
  it) by re-checking the actual production database after the script reported success rather than
...
- **`pool.end()` isn't awaited before `process.exit()` in the seed scripts and `migrate.js`'s CLI
  ...
  Railway's TCP proxy — after the actual seeding work had already finished successfully. Removed
  entirely rather than given a timeout, since there was nothing it was buying.
...
**`seed:compendium` and `seed:characters` are pure database writes** — no files involved, so pointing
them at a remote `DATABASE_URL` from your own machine works exactly like pointing them at local
SQLite:
...
Both skip/update in place rather than duplicating: Compendium `source = 'core'` rows are matched and
refreshed, never re-inserted; `seed:characters` matches existing characters by name and skips them
with a warning, so it's safe to run again after adding a new player to the source file.

**`seed:map` is different — it also writes image files to local disk, so only run it against a
database that lives on the same machine as those files.** Locally, that's true by definition and
`npm run seed:map` works as expected. Against Railway, it is *not*: the script would create the
derivative `.webp` files on your machine while writing their filenames into Railway's Postgres, and
the live app would have no way to actually serve them — a calibration that looks complete in the
database but 404s for every player. (This happened once — see "Judgment calls".) **For Railway, GM
the map upload through the live app itself instead**: log in with the GM passcode, open Map
Calibration, upload the image there. That request is handled by Railway's own running server, so the
derivatives land in the right place the first time.

**Running `seed:compendium`/`seed:characters` against Railway** from your own machine, e.g. in
PowerShell:

```powershell
$env:DB_CLIENT = "pg"
$env:DATABASE_URL = "<Railway Postgres connection string — see below>"
npm run seed:compendium
npm run seed:characters -- "C:\path\to\characters_seed.json"
```

Railway's Postgres service doesn't expose a public connection string by default — its `DATABASE_URL`
only resolves from inside Railway's own network. To get one reachable from your machine: open the
Postgres service → **Settings → Networking**, enable the **TCP Proxy**, then either use the
`DATABASE_PUBLIC_URL` variable it adds (click its value to reveal the resolved string — Railway
sometimes displays the unresolved `${{...}}` reference syntax instead) or build it from the
individual `PGUSER`/`PGPASSWORD`/`PGDATABASE` values plus the new proxy host/port it shows you:
`postgresql://<user>:<password>@<proxy-host>:<proxy-port>/<database>`. **Turn the TCP Proxy back off
once you're done** — there's no reason to leave a database reachable from the open internet after a
one-time seed; the running app talks to it over Railway's internal network regardless.

These commands can take noticeably longer against Railway's proxy than against local SQLite (real
network latency per query, and `seed:characters` re-runs the full Compendium seed via `migrate()`
first) — a minute or two is normal, not a hang.

Unset `$env:DB_CLIENT`/`$env:DATABASE_URL` afterward (or just close the terminal) so your next
`npm run dev` goes back to local SQLite.

**Uploaded files (the map, Handouts) need their own persistence story, separate from the database.**
`uploads/` is a local-disk directory; on Railway that's the container's filesystem, which resets on
every redeploy same as SQLite would. If you want the map/Handouts to survive a `git push`, attach a
Railway **Volume** to the *web* service (not just Postgres) mounted at the path the app writes
uploads to — `/app/uploads` under Railway's default Node/Nixpacks build. Without one, re-upload the
map through the GM screen after each deploy.
